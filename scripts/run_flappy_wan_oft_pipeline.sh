...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "[flappy-wanoft-fixed] Unknown argument: $1" >&2
      usage >&2
      exit 2
      ;;
  esac
...
if ! [[ "${CONTEXT_WINDOW}" =~ ^[0-9]+$ ]] || [[ "${CONTEXT_WINDOW}" -lt 2 ]]; then
  echo "[flappy-wanoft-fixed] --context-window must be an integer >= 2, got: ${CONTEXT_WINDOW}" >&2
  exit 2
fi
if ! [[ "${MAX_EPISODES}" =~ ^[0-9]+$ ]] || [[ "${MAX_EPISODES}" -lt 1 ]]; then
  echo "[flappy-wanoft-fixed] --max-episodes must be a positive integer, got: ${MAX_EPISODES}" >&2
  exit 2
fi
...
CONVERTED_DATA_ROOT="data/flappy_fix_latency_${LATENCY}_${MAX_EPISODES}ep_context${CONTEXT_WINDOW}"
CONVERTED_DATA_DIR="${CONVERTED_DATA_ROOT}/flappy_train__bridge"
...
ensure_hf_cli() {
  if ! command -v hf >/dev/null 2>&1; then
    echo "[flappy-wanoft-fixed] Hugging Face CLI command 'hf' is not available in PATH." >&2
    echo "[flappy-wanoft-fixed] Re-run without --skip-env-setup, or install huggingface-hub in ${CONDA_ENV_NAME}." >&2
    exit 1
  fi
}
...
activate_conda_env() {
  if ! command -v conda >/dev/null 2>&1; then
    echo "[flappy-wanoft-fixed] conda is required but was not found in PATH." >&2
    exit 1
  fi
  local conda_base
...
for field in ("latency_raw_frames", "latency_env_steps", "latency_filter"):
    if manifest.get(field) != [3]:
        raise ValueError(
            f"Expected manifest {field}=[3], got {manifest.get(field)!r}"
        )
if set(prompt_map) != {"3"}:
    raise ValueError(
        f"Expected exactly latency 3 in the prompt map, got {sorted(prompt_map)}"
    )
if prompt_map["3"].get("latency_raw_frames") != 3:
    raise ValueError(
        "Expected prompt-map latency_raw_frames=3, got "
        f"{prompt_map['3'].get('latency_raw_frames')!r}"
    )
PY
}
...
if [[ "${SKIP_CHECKPOINTS}" != "true" ]]; then
  echo "[flappy-wanoft-fixed] Downloading Wan base model checkpoint"
  hf download "${BASE_MODEL_REPO}" \
    --local-dir "${BASE_MODEL_DIR}"

  echo "[flappy-wanoft-fixed] Downloading WanOFT initialization checkpoint"
  hf download "${INIT_CHECKPOINT_REPO}" \
    --local-dir "${INIT_CHECKPOINT_DIR}"
fi

if [[ "${SKIP_CONVERT}" != "true" ]]; then
  echo "[flappy-wanoft-fixed] Converting ${DATASET_REPO}/${DATASET_CONFIG} into ${CONVERTED_DATA_DIR}"
  CONVERTER_ARGS=(
    python
...
if [[ "${SKIP_TRAIN}" != "true" ]]; then
  validate_converted_dataset
  echo "[flappy-wanoft-fixed] Training latency=${LATENCY} run_id=${RUN_ID}"
...
if [[ "${SKIP_UPLOAD}" != "true" ]]; then
  if [[ ! -d "${RUN_DIR}" ]]; then
    echo "[flappy-wanoft-fixed] Training output directory does not exist: ${RUN_DIR}" >&2
    exit 1
  fi
  echo "[flappy-wanoft-fixed] Uploading ${RUN_DIR} to ${UPLOAD_REPO}:${UPLOAD_PATH_IN_REPO}"
  hf upload "${UPLOAD_REPO}" "${RUN_DIR}" "${UPLOAD_PATH_IN_REPO}" \
    --exclude "wandb/**" \
    --repo-type model
fi

echo "[flappy-wanoft-fixed] Complete: latency=${LATENCY} run_id=${RUN_ID}"
