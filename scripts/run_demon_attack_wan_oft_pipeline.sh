...
Options:
  ...
  --upload-repo <repo>        HF model repo for run upload (default: placeholder/demon_attack_200ep)
...
RUN_ID="${RUN_ID:-}"
UPLOAD_REPO="${UPLOAD_REPO:-placeholder/demon_attack_200ep}"
...
if [[ "${SKIP_ENV_SETUP}" != "true" ]]; then
  if [[ ! -f "${BENCHMARK_ROOT}/pyproject.toml" ]] \
    ...
    echo "[demon-wanoft-fixed] Invalid placeholder checkout: ${BENCHMARK_ROOT}" >&2
