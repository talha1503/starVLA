...
bash examples/rl_games/install/install_stack.sh pi05 defend_the_line
...
python examples/rl_games/scripts/launch_train.py \
    ...
    init=bridge \
    mode=single \
    run_id="pi05_bridge_defend_the_line_single_latency_clean_data_exp1" \
    trainer.distributed_backend=none \
    workspace_dir="$WORKSPACE_DIR" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/pi05_bridge_defend_the_line_single_latency_clean_data_exp1" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/pi05_bridge_defend_the_line_single_latency_clean_data_exp1" \
