...
bash examples/rl_games/install/install_stack.sh openvla asterix
...
# Native/scratch Asterix keeps the original 9-way Atari action head. Use
# openvla/bridge/single/asterix.sh for the Bridge-compatible factorized 6-logit
# layout.
python examples/rl_games/scripts/launch_train.py \
    ...
    init=scratch \
    mode=single \
    run_id="openvla_asterix_single_latency_clean_data_exp1" \
    trainer.distributed_backend=none \
    workspace_dir="$WORKSPACE_DIR" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_asterix_single_latency_clean_data_exp1" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/openvla_asterix_single_latency_clean_data_exp1" \
    dataset.source_hf=placeholder/memory-rollouts \
    dataset.source_subdir=asterix_fixed_latency_0_1000ep_7k2steps \
