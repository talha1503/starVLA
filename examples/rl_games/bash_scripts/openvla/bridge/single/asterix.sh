...
python examples/rl_games/scripts/launch_train.py \
    ...
    init=bridge \
    mode=single \
    run_id="openvla_bridge_asterix_factorized6_single_latency_clean_data_exp1" \
    trainer.distributed_backend=none \
    workspace_dir="$WORKSPACE_DIR" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_bridge_asterix_factorized6_single_latency_clean_data_exp1" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/openvla_bridge_asterix_factorized6_single_latency_clean_data_exp1" \
