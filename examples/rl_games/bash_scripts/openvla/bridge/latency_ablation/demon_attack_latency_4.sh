...
python examples/rl_games/scripts/launch_train.py \
    ...
    init=bridge \
    mode=single \
    run_id="openvla_demon_attack_single_latency_ablation_exp1" \
    trainer.distributed_backend=none \
    workspace_dir="/workspace" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_demon_attack_single_latency_ablation_exp1" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/openvla_demon_attack_single_latency_ablation_exp1" \
