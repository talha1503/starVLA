...
python examples/rl_games/scripts/launch_train.py \
    ...
    init=bridge \
    mode=cross_task \
    cross_task_setup=defendtheline_zero_deadly_zero \
    run_id="openvla_bridge_cross_defendtheline_zero_deadly_zero_exp3" \
    trainer.distributed_backend=none \
    workspace_dir="$WORKSPACE_DIR" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_bridge_cross_defendtheline_zero_deadly_zero_exp3" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/openvla_bridge_cross_defendtheline_zero_deadly_zero_exp3" \
    ...
    rl_games.cross_task.train_tasks.0.name=defend_the_line \
    rl_games.cross_task.train_tasks.0.converted_name=defend_the_line_0_cross_train_openvla_bridge_cross_defendtheline_zero_deadly_zero_exp3 \
    rl_games.cross_task.train_tasks.0.train_source_hf=placeholder/memory-rollouts \
    rl_games.cross_task.train_tasks.0.prompt_source_hf=placeholder/memory-rollouts \
    rl_games.cross_task.train_tasks.0.train_source_subdir=defend_the_line_fixed_latency_0_1000ep_7k2steps \
    rl_games.cross_task.train_tasks.0.prompt_source_subdir=defend_the_line_fixed_latency_0_1000ep_7k2steps \
    rl_games.cross_task.train_tasks.0.train_latency_filter=[0] \
    rl_games.cross_task.train_tasks.0.eval_latency_filter=[0] \
    rl_games.cross_task.train_tasks.0.episodes_per_latency=1000 \
    rl_games.cross_task.train_tasks.1.name=deadly_corridor \
    rl_games.cross_task.train_tasks.1.converted_name=deadly_corridor_0_cross_train_openvla_bridge_cross_defendtheline_zero_deadly_zero_exp3 \
    ...
    rl_games.cross_task.eval_tasks.defend_the_line.post_train.latencies=[0,2,4] \
    rl_games.cross_task.eval_tasks.defend_the_line.post_train.num_episodes=20 \
    rl_games.cross_task.eval_tasks.defend_the_line.post_train.max_steps_per_episode=3600 \
