...
python examples/rl_games/scripts/launch_train.py \
    ...
    run_id="openvla_bridge_cross_demon_024_flappy_zero_clean_data_exp3" \
    trainer.distributed_backend=none \
    workspace_dir="/workspace" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_bridge_cross_demon_024_flappy_zero_clean_data_exp3" \
    checkpoint.sync.enabled=true \
    datasets.vla_data.sequential_step_sampling=true \
    datasets.vla_data.shuffle=true \
    checkpoint.sync.repo_id="placeholder/openvla_bridge_cross_demon_024_flappy_zero_clean_data_exp3" \
