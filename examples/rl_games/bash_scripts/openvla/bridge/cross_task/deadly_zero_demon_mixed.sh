...
python examples/rl_games/scripts/launch_train.py \
    ...
    init=bridge \
    mode=cross_task \
    cross_task_setup=deadly_zero_demon_mixed \
    run_id="openvla_bridge_cross_deadly_zero_demon_024_exp1" \
    trainer.distributed_backend=none \
    workspace_dir="/workspace" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_bridge_cross_deadly_zero_demon_024_exp1" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/openvla_bridge_cross_deadly_zero_demon_024_exp1" \
    ...
    rl_games.cross_task.train_tasks.0.converted_name=demon_attack_200ep_cross_0_train \
