...
python examples/rl_games/scripts/launch_train.py \
    ...
    run_id="openvla_bridge_cross_flappy_0_demon_0_clean_data_exp1" \
    trainer.distributed_backend=none \
    workspace_dir="/workspace" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_bridge_cross_flappy_0_demon_0_clean_data_exp1" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/openvla_bridge_cross_flappy_0_demon_0_clean_data_exp1" \
    checkpoint.save_best_model=false \
    datasets.vla_data.sequential_step_sampling=true \
    datasets.vla_data.shuffle=true \
    trainer.max_train_steps=3375 \
    trainer.num_warmup_steps=0 \
    trainer.eval_interval=700 \
    trainer.save_interval=3375 \
    ...
    rl_games.cross_task.train_tasks.1.converted_name=flappy_0_200ep_cross_train \
    rl_games.cross_task.train_tasks.1.train_source_hf=placeholder/flappy_200ep \
    rl_games.cross_task.train_tasks.1.prompt_source_hf=placeholder/flappy_200ep \
