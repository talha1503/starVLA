...
python examples/rl_games/scripts/launch_train.py \
    ...
    init=bridge \
    mode=mixed_latency \
    run_id="openvla_demon_attack_mixed_latency_clean_exp2" \
    trainer.distributed_backend=none \
    workspace_dir="/workspace" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_demon_attack_mixed_latency_clean_exp2" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/openvla_demon_attack_mixed_latency_clean_exp2" \
