...
python examples/rl_games/scripts/launch_train.py \
    ...
    init=bridge \
    mode=single \
    run_id="openvla_bridge_deadly_corridor_fix_latency_6_clean_data_bce_exp2" \
    trainer.distributed_backend=none \
    workspace_dir="/workspace" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/openvla_bridge_deadly_corridor_fix_latency_6_clean_data_bce_exp2" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/openvla_bridge_deadly_corridor_fix_latency_6_clean_data_bce_exp2" \
    dataset.source_hf=placeholder/deadly_1000ep \
    dataset.source_subdir=deadly_corridor_fix_latency_6_1000ep \
