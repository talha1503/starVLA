...
from examples.rl_games.bash_scripts.gr00t.data_conversion import (
    convert_demon_attack_to_starvla_lerobot as demon_attack_converter,
)
...
def _validate_source_row(
    ...
    if str(row["env_name"]) != "demon_attack":
        raise ValueError(
            f"Expected env_name='demon_attack', got {row['env_name']!r}"
        )
...
def _convert_episode(
    ...
    if not source_rows:
        raise ValueError("Cannot convert an empty Demon Attack episode")
    ...
    for frame_idx, row in enumerate(source_rows):
        ...
        image_bytes = demon_attack_converter._png_bytes(row["image"])
        if first_image_bytes is None:
            first_image_bytes = image_bytes
            image_shape = demon_attack_converter._image_shape(image_bytes)
        ...
        out_rows.append(
            {
                ...
                "action": demon_attack_converter._one_hot(
                    int(row["action_id"]),
    ...
    if image_shape is None:
        raise ValueError("Converted Demon Attack episode has no image shape")
...
def _convert_split(
    ...
    action_dim = demon_attack_converter._action_dim(action_carrier)
    action_labels = demon_attack_converter._action_labels(action_carrier)
    state_dim = demon_attack_converter._state_dim(action_carrier)
    state_labels = demon_attack_converter._state_labels(action_carrier)
    prompt_to_task_index: dict[str, int] = {}
    task_prompts: list[str] = []
    latency_prompt_entries: dict[tuple[int, str], dict[str, Any]] = {}
    ...
    def _write_current_episode() -> None:
        ...
        demon_attack_converter._write_episode(
            split_output_dir
    ...
    demon_attack_converter._write_metadata(
        split_output_dir,
    ...
    latency_prompt_map = demon_attack_converter.build_latency_prompt_map(
    ...
    manifest = {
        ...
        "action_labels": action_labels,
        "action_dim": action_dim,
        "active_action_dim": demon_attack_converter.ACTION_DIM,
        "action_carrier": action_carrier,
        "bridge_action_dim": (
            demon_attack_converter.BRIDGE_ACTION_DIM
            if action_carrier == "bridge"
            else None
        ),
        "state_dim": state_dim,
        "active_state_dim": demon_attack_converter.STATE_DIM,
...
def convert_hub_dataset(
    ...
    action_carrier = demon_attack_converter._normalize_action_carrier(action_carrier)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Convert row-history Demon Attack rollouts directly into the existing "
    ...
    parser.add_argument(
        ...
        default=demon_attack_converter.DEFAULT_CONTEXT_IMAGES_OUTPUT_COLUMN,
    )
