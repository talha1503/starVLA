...
python examples/rl_games/scripts/launch_train.py \
    ...
    init=bridge \
    mode=single \
    run_id="gr00t_bridge_deadly_corridor_single_latency_clean_data_exp2_action_1e-4_backbone_1e-5" \
    trainer.distributed_backend=none \
    workspace_dir="/workspace" \
    wandb_entity="anonymous" \
    checkpoint.hf_repo_id="placeholder/gr00t_bridge_deadly_corridor_single_latency_clean_data_exp2_action_1e-4_backbone_1e-5" \
    checkpoint.sync.enabled=true \
    checkpoint.sync.repo_id="placeholder/gr00t_bridge_deadly_corridor_single_latency_clean_data_exp2_action_1e-4_backbone_1e-5" \
