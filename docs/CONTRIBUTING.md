...
- **Report a bug** — Open an [Issue](https://github.com/placeholder/placeholder/issues). If it needs more context, start a Discussion.
...
- **Before submitting a PR**, run Black + Ruff on the files you changed (the repo currently has a historical lint backlog, so full-repo `make check` is expected to fail; see [PR_readme.md](PR_readme.md#4-pre-submit-checklist)).
