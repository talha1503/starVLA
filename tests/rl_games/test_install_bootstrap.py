...
def test_bootstrap_and_install_stack_accept_world_model_targets() -> None:
    ...
    assert "openvla|pi0|pi05|gr00t|wan_oft|cosmo_predict_gr00t|all" in bootstrap_source
    assert "MODELS=(openvla pi0 pi05 gr00t wan_oft cosmo_predict_gr00t)" in bootstrap_source
    assert "openvla|pi0|pi05|gr00t|wan_oft|cosmo_predict_gr00t" in install_stack_source
...
def test_cosmo_predict_gr00t_install_assets_are_registered() -> None:
    repo_root: Path = _repo_root()
    install_dir: Path = repo_root / "examples" / "rl_games" / "install"
    model_installer: Path = install_dir / "model" / "cosmo_predict_gr00t.sh"
    flappy_validator: Path = install_dir / "validate" / "cosmo_predict_gr00t_flappy.sh"
    flappy_validator_source: str = flappy_validator.read_text()
    ...
    assert flappy_validator.is_file()
    assert os.access(flappy_validator, os.X_OK)
    assert "from diffusers import AutoencoderKLWan, CosmosTransformer3DModel" in flappy_validator_source
    assert "from transformers import T5EncoderModel" in flappy_validator_source
