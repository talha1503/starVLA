...
# Usage:
#   bash commands/wanoft/train_defend_the_line_wan_oft.sh
#   LATENCY=0 bash commands/wanoft/train_defend_the_line_wan_oft.sh
LATENCY="${1:-${LATENCY:-0}}"
if [[ $# -gt 0 ]]; then
  shift
fi
...
DATASET_LOCAL_DIR="${DATASET_LOCAL_DIR:-data/defend_the_line_fix_latency_${LATENCY}_${MAX_EPISODES}ep_context${CONTEXT_WINDOW}}"
RUN_ID="${RUN_ID:-wan_oft_defend_the_line_fix_latency_${LATENCY}_context${CONTEXT_WINDOW}_standard_sft_${MAX_TRAIN_STEPS}_effbs${EFFECTIVE_BATCH_SIZE}_224_currentce}"
PROMPT_MAP_PATH="${PROMPT_MAP_PATH:-${DATASET_LOCAL_DIR}/defend_the_line_train__bridge/latency_prompt_map.json}"
...
python examples/rl_games/scripts/launch_train.py \
  ...
  dataset.converted_name=defend_the_line_train__bridge \
  trainer.distributed_backend=none \
  launch.use_accelerate=false \
  launch.num_processes=1 \
  trainer.gradient_accumulation_steps="${GRADIENT_ACCUMULATION_STEPS}" \
  datasets.vla_data.per_device_batch_size="${PER_DEVICE_BATCH_SIZE}" \
  datasets.vla_data.data_mix=defend_the_line_train__bridge \
