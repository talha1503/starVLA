...
# Streaming-KV full tuning with packed replay. The dataloader emits the same
# eight-frame rollout and per-frame labels as serial replay, but FlexAttention
# packs the rollout into one forward while preserving the four-frame streaming
# attention window. Packed replay retains full temporal gradient flow instead
# of detaching cached states between frames.
#
