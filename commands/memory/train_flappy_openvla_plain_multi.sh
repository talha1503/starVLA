...
# Four-frame visual-prompt full tuning. Each sample supplies the four newest
# observations as separate images in one prompt and supervises one action
# vector; no persistent cache or temporal replay is used.
#
# DeepSpeed ZeRO-2 runs on two processes. Effective batch:
# per_device_batch_size * num_processes * gradient_accumulation_steps
# = 16 * 2 * 4 = 128 prompts and action labels per optimizer step.
