...
# Usage:
#   bash commands/pi05/train_demon_attack_pi05.sh 2
#   LATENCY=2 bash commands/pi05/train_demon_attack_pi05.sh
LATENCY="${1:-${LATENCY:-0}}"
if ! [[ "${LATENCY}" =~ ^[0-9]+$ ]]; then
...
RUN_ID="${RUN_ID:-pi05_demon_attack_fix_latency_${LATENCY}_${MAX_EPISODES}ep}"
